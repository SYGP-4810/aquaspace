...
class Common extends \Core\Controller
{
    ...
    public function getLandingPagePostAction()
    {
        ...
        $stmtForFish = $this->execute("SELECT products.id,products.product_name, COUNT(product_order.product_id) AS countOfRating, SUM(selling_order.rating) AS sumOfRating, products.price,products.img1 FROM selling_order,product_order,products WHERE status='1' products.id=product_order.product_id AND selling_order.id=product_order.selling_order_id AND products.type='1' GROUP BY products.id ORDER BY products.id DESC LIMIT 4;");
        $stmtForPlant = $this->execute("SELECT products.id,products.product_name, COUNT(product_order.product_id) AS countOfRating, SUM(selling_order.rating) AS sumOfRating, products.price,products.img1 FROM selling_order,product_order,products WHERE status='1' products.id=product_order.product_id AND selling_order.id=product_order.selling_order_id AND products.type='2' GROUP BY products.id ORDER BY products.id DESC LIMIT 4;");
        $stmtForeq = $this->execute("SELECT products.id,products.product_name, COUNT(product_order.product_id) AS countOfRating, SUM(selling_order.rating) AS sumOfRating, products.price,products.img1 FROM selling_order,product_order,products WHERE  status='1' products.id=product_order.product_id AND selling_order.id=product_order.selling_order_id AND products.type='3' GROUP BY products.id ORDER BY products.id DESC LIMIT 4;");
    ...
    public function getCategoryPostAction()
    {
        ...
        $stmtForAll = $this->execute("SELECT products.id,products.product_name, products.delivery,products.lat,products.lan, products.price,products.img1 FROM products WHERE products.type !='4'AND status='1' ORDER BY products.id DESC ");
        $stmtForFish = $this->execute("SELECT products.id,products.product_name, products.delivery, products.lat,products.lan, products.price,products.img1 FROM products WHERE products.type='1' AND status='1' ORDER BY products.id DESC;");
        $stmtForPlant = $this->execute("SELECT products.id,products.product_name, products.delivery, products.lat,products.lan, products.price,products.img1 FROM products WHERE products.type='2' AND status='1' ORDER BY products.id DESC;");
        $stmtForEq = $this->execute("SELECT products.id,products.product_name, products.delivery, products.lat,products.lan, products.price,products.img1 FROM products WHERE products.type='3' AND status='1' ORDER BY products.id DESC;");
